...
const adminNav = [
  ...
  { to: '/admin/courses', icon: '📚', label: 'Lớp học' }
]
    ...
    <div className="min-h-screen bg-surface-950 flex">
      ...
      <aside className={`${collapsed ? 'w-16' : 'w-60'} shrink-0 bg-surface-900 border-r border-surface-700 flex flex-col transition-all duration-200 sticky top-0 h-screen`}>
        ...
        <div className="p-3 border-t border-surface-700">
          {!collapsed && (
            <div className="flex items-center gap-3 px-3 py-2.5 mb-2 rounded-lg bg-surface-800">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-text-primary text-xs font-medium truncate">{user?.fullName || user?.username}</p>
                <span className={`${roleColor} text-[10px] px-1.5 py-0 border rounded-full`}>{roleLabel}</span>
              </div>
            </div>
          )}
