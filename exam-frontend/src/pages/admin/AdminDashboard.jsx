...
const Icon = {
  ...
  post:     <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 8.25h9m-9 3H12m-9.75 1.51c0 1.6 1.123 2.994 2.707 3.227 1.129.166 2.27.293 3.423.379.35.026.67.21.865.501L12 21l2.755-4.133a1.14 1.14 0 01.865-.501 48.172 48.172 0 003.423-.379c1.584-.233 2.707-1.626 2.707-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z"/></svg>,
  trending: <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 18L9 11.25l4.306 4.307a11.95 11.95 0 015.814-5.519l2.74-1.22m0 0l-5.94-2.28m5.94 2.28l-2.28 5.941"/></svg>,
}
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    api.get('/dashboard/discussion/admin')
      ...
      {!discussionLoading && discussionStats && (
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="card p-4 flex items-center gap-4">
              <div className="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0"
                style={{ background: '#7551FF20', color: '#7551FF' }}>
                {Icon.post}
              </div>
              <div>
                <p className="text-xs" style={{ color: 'var(--text-3)' }}>Tổng thảo luận</p>
                <p className="font-bold text-lg" style={{ color: 'var(--text-1)' }}>
                  {discussionStats.totalPosts}
                  <span className="text-sm font-normal ml-1.5" style={{ color: 'var(--text-3)' }}>
                    bài · {discussionStats.totalReplies} phản hồi
                  </span>
                </p>
              </div>
            </div>
            ...
            <div className="card p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs" style={{ color: 'var(--text-3)' }}>Hoạt động tuần này</p>
                <p className="font-bold text-lg" style={{ color: 'var(--text-1)' }}>
                  {discussionStats.weeklyPosts}
                  <span className="text-sm font-normal ml-1.5" style={{ color: 'var(--text-3)' }}>bài mới</span>
                </p>
              </div>
            </div>
            ...
            <div className="card p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs" style={{ color: 'var(--text-3)' }}>Khóa tích cực nhất</p>
                <p className="font-bold text-sm leading-tight" style={{ color: 'var(--text-1)' }}>
                  {discussionStats.topCourseName || '—'}
                </p>
                {discussionStats.topCoursePostCount > 0 && (
                  <p className="text-xs mt-0.5" style={{ color: 'var(--text-3)' }}>
                    {discussionStats.topCoursePostCount} bài viết
                  </p>
                )}
              </div>
            </div>
