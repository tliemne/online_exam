...
function AddQuestionsModal({ exam, onClose, onSaved }) {
  ...
  // scores: questionId → điểm tối đa (mặc định từ exam_questions nếu đã có, không thì 1.0)
  const [scores, setScores] = useState(() => {
    const init = {}
    ;(exam.questions || []).forEach(q => {
      init[q.questionId || q.id] = q.score ?? 1.0
    })
    return init
  })
  ...
  const toggle = (id, type) => {
    ...
    setSelected(s)
    // Mặc định điểm: tự luận = 2.0, còn lại = 1.0
    if (!s.has(id)) return
    if (!(id in scores)) {
      setScores(p => ({ ...p, [id]: type === 'ESSAY' ? 2.0 : 1.0 }))
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (toAdd.length > 0) {
        // Lấy question type để xác định default score
        const qTypeMap = {}
        questions.forEach(q => { qTypeMap[q.id] = q.type })

        const items = toAdd.map((qId, i) => {
          const raw = scores[qId]
          const parsed = parseFloat(raw)
          const defaultScore = qTypeMap[qId] === 'ESSAY' ? 2.0 : 1.0
          return {
            questionId: qId,
            score: (!isNaN(parsed) && parsed > 0) ? parsed : defaultScore,
            orderIndex: i + 1
          }
        })
              ...
              return (
                <div key={q.id}
                  className={`rounded-lg border transition-all ${
                    isSelected ? 'border-accent bg-accent/8' : 'border-surface-600 bg-surface-700 hover:border-surface-500'
                  }`}>
                  {/* Câu hỏi row */}
                  <div className="flex items-start gap-3 p-3 cursor-pointer" onClick={() => toggle(q.id, q.type)}>
                    <div className={`w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 mt-0.5 transition-colors ${
                      isSelected ? 'bg-accent border-accent' : 'border-surface-400'
                    }`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-2 mt-1.5">
                        <span className={`text-xs ${isEssay ? 'text-purple-400 font-medium' : 'text-text-muted'}`}>
                          {TYPE_META[q.type] || q.type}
                        </span>
                        {q.difficulty && (
                  ...
                  {/* Score input — chỉ hiện khi đã tick */}
                  {isSelected && (
                    <div className="flex items-center gap-2 px-3 pb-3" onClick={e => e.stopPropagation()}>
                      <span className="text-xs text-text-muted">Điểm câu này:</span>
                      <input
                        type="number" min="0.5" step="0.5"
                        value={scores[q.id] ?? (isEssay ? 2.0 : 1.0)}
                        onChange={e => setScore(q.id, e.target.value)}
                        className="input-field py-1 w-20 text-sm text-center"
                      />
        ...
        {selected.size > 0 && (
          <div className="flex items-center justify-between text-sm px-1">
            <span className="text-text-muted">
              Tổng điểm {selected.size} câu:
            </span>
            <span className="font-semibold text-accent">
              {[...selected].reduce((sum, id) => sum + parseFloat(scores[id] || 1.0), 0).toFixed(1)} điểm
              {exam.totalScore && (
                <span className="text-text-muted font-normal">
                  {' '}(đề tối đa {exam.totalScore}đ — tỉ lệ tự động quy đổi)
                </span>
              )}
            </span>
          </div>
