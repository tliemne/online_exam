...
export default function TeacherStatsPage() {
  ...
  const statCards = [
    { label: t('stats.totalExams'),   value: exams.length,     color: 'var(--accent)' },
    { label: t('stats.published'),       value: published.length, color: '#22c55e' },
    { label: t('stats.draft'),      value: draft.length,     color: '#f59e0b' },
    { label: t('stats.closed'),       value: closed.length,    color: '#6b7280' },
    { label: t('stats.courses'),       value: courses.length,   color: '#8b5cf6' },
  ]
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-[var(--text-3)] mt-1">{t('stats.subtitle')}</p>
      </div>
      ...
      <div>
        <h2 className="font-semibold text-[var(--text-1)] mb-3">{t('stats.recentExams')}</h2>
        <div className="space-y-2">
          {exams.slice(0, 10).map(exam => {
            const statusMap = {
              PUBLISHED: { label: t('stats.published'),  color: '#22c55e', bg: 'rgba(34,197,94,0.1)'   },
              DRAFT:     { label: t('stats.draft'),     color: '#f59e0b', bg: 'rgba(245,158,11,0.1)'  },
              CLOSED:    { label: t('stats.closed'),  color: '#6b7280', bg: 'rgba(107,114,128,0.1)' },
            }
            ...
            return (
              <div key={exam.id} className="card p-4 flex items-center gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[var(--text-3)]">{exam.courseName} · {exam.questionCount ?? 0} {t('stats.questions')} · {exam.durationMinutes} {t('stats.minutes')}</p>
                </div>
        ...
        {exams.length > 10 && (
          <p className="text-center text-xs text-[var(--text-3)] mt-3">
            {t('stats.and')} {exams.length - 10} {t('stats.otherExams')}
            <button onClick={() => navigate('/teacher/exams')} className="ml-1 text-accent hover:underline">{t('stats.viewAll')}</button>
          </p>
        )}
      </div>
      ...
      {/* Courses */}
      <div>
        <h2 className="font-semibold text-[var(--text-1)] mb-3">{t('stats.courses')} ({courses.length})</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {courses.map(c => (
            <div key={c.id} className="card p-4 cursor-pointer hover:border-accent/40 transition-colors"
              ...
              <p className="text-xs text-[var(--text-3)] mt-1">{c.studentCount ?? 0} {t('stats.students')}</p>
            </div>
          ))}
        </div>
      </div>
